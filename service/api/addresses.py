...
def create_address(payload, person_id):
    ...
    # ahead and create with no additional logic.
        ...
        most_recent_address_date = max(addSeg.start_date for addSeg in person.address_segments)

        if payload.get("start_date") is None or payload.get("start_date") <= most_recent_address_date:
            abort(404, description="invalid start date")
        else:
            address_segment = AddressSegment(
            street_one=payload.get("street_one"),
            street_two=payload.get("street_two"),
        ...
        previous_address_segment =  max(person.address_segments, key=attrgetter('start_date'))
        previous_address_segment.end_date = payload.get("start_date")
